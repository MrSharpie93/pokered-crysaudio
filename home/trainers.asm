...
.nonZeroOffset
	...
	cp $a
	jr nz, .done
	; ~$~FIXED: TrainerEndBattleText 2 isn't overwritten.~$~
.readPointer
	ld a, [hli]
	ld h, [hl]
	ld l, a
...
.trainerNotYetFought
	...
	call PrintText
	ld a, $a
	call ReadTrainerHeaderInfo     ; ~$~FIXED~$~read end battle text (2)
	push de
	ld a, $8
...
SaveTrainerName:: ; ~$~CHANGED: Removed redundant function, and put it here.~$~
	ld hl, wTrainerName
	ld de, wNameBuffer
.CopyCharacter
	ld a, [hli]
...
.femaleTrainerListLoop
	...
	jr .PlaySound
.noFemaleTrainer
	ld hl, WeirdTrainerList
.weirdTrainerListLoop
	ld a, [hli]
	cp $ff
	jr z, .noWeirdTrainer
	cp b
	jr nz, .weirdTrainerListLoop
