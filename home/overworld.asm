...
; if A is pressed
	...
	ldh a, [hTextID]
	and a
	jr nz, .displayDialogue ; ~$~CHANGED: Overworld HM usage.~$~
	predef TryFieldMove
	jp OverworldLoop
.displayDialogue
...
.currentTileMatchesFirstInPair
	inc hl
	ld a, [hli] ; ~$~FIXED: Tile collision pairs function correctly.~$~
	cp c
	jr z, .foundMatch
	jr .tilePairCollisionLoop
...
CollisionCheckOnWater::
	...
	ld a, [wSpritePlayerStateData1CollisionData]
	and d ; check if a sprite is in the direction the player is trying to go
	jr nz, .collision ; ~$~FIXED: Yellow fix for water collision issues.~$~
