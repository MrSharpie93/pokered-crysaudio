AskName:
	...
	call ReloadMapData ; ~$~CHANGED: New icons. Code from Rangi42 and ShantyTown.~$~
.inBattle
...
.selectReturnPoint
	call PrintAlphabet
; ~$~CHANGED: Dannye mod for full-color icons.~$~
	ld a, %11100100 ; 3210
	ldh [rBGP], a
	ldh [rOBP0], a
.ABStartReturnPoint
...
;	ld a, [wNamingScreenLetter] ;~$~REMOVED: English language does not use these.~$~
;	cp "ﾞ"
;	ld de, Dakutens
...
;.dakutensAndHandakutens ;~$~REMOVED: English language does not use these.~$~
;	push hl
...
LoadEDTile:
	ld de, ED_Tile
	ld hl, vFont tile $70
	; ~$~FIXED~$~BUG: BANK("Home") should be BANK(ED_Tile), although it coincidentally works as-is
...
PrintNamingText:
	...
	hlcoord 4, 1
	call PlaceString
	ld hl, $0 ; For whatever reason, this has to be here to make this display correctly.~$~
	add hl, bc
	ld [hl], "'s" ;"の" ; leftover from Japanese version; blank tile $c9 in English
	hlcoord 1, 3
