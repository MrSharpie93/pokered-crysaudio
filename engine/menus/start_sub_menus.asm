...
;	jp z, .useOrTossItem
;.notBicycle1
	ld a, USE_INFO_TOSS_MENU_TEMPLATE ; ~$~CHANGED: Item descriptions.~$~
	ld [wTextBoxID], a
...
.useOrTossItem ; if the player made the choice to use or toss the item
	...
	ld a, [wCurrentMenuItem] ; ~$~CHANGED: Item descriptions.~$~
	cp a, 2
	jr z, .tossItem
	cp a, 1
	jp z, .infoItem
; use item
	...
	call PrintText
	jp ItemMenuLoop
.notBicycle ; ~$~CHANGED: Item descriptions.~$~
	xor a
	ld [wPseudoItemID], a
	ld a, [wCurItem]
	cp HM01
...
StartMenu_TrainerInfo::
	...
	ld a, [wOnSGB] ; ~$~FIXED: No garbage data on DMG.~$~
	and a
	call z, Delay3
	call GBPalNormal
