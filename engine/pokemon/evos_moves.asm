...
EvolutionAfterBattle:
	...
	push bc
	push de
	ld hl, wStartBattleLevels ; ~$~FIXED: No weird item evolutions.~$~
	push hl
	ld hl, wPartyCount
	push hl
...
Evolution_PartyMonLoop: ; loop over party mons
	...
	inc [hl]
	pop hl
	pop de ; ~$~FIXED: No weird item evolutions.~$~
	ld a, [de]
	ld [wTempCoins1], a
	inc de
	inc hl
...
; ~$~ CHANGED: Pokemon can learn multiple moves at the same level.~$~
...
; ~$~Unrelated, but the above is absolutely my favorite programming comment ever and exactly matches my experience trying to get this thing to work before this tutorial was added.~$~
...
.knowsMove
	...
	pop bc
	jr .loop
.done ; ~$~CHANGED: Removed portion of code for relearning level 1 moves to accommodate Move Tutors.~$~
	ld b, 0
	ld hl, wMoveBuffer + 1
	add hl, bc
	ld a, $ff
