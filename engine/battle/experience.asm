...
;	call DivideExpDataByNumMonsGainingExp ; ~$~CHANGED: Reworks to EXP system.~$~
...
.skipExpText
	...
	call LoadMonData
	call AnimateEXPBar ; ~$~ADDED: EXP bar in battle.~$~
	pop hl
	...
	ld [wTempLevel], a ; ~$~CHANGED: Ported KEP's port of Red++'s level-up move learning code.~$~
	cp d
	jp z, .nextMon ; if level didn't change, go to next mon
	call KeepEXPBarFull ; ~$~ADDED: EXP bar in battle.~$~
	ld a, [wCurEnemyLevel]
	push af
	push hl
	ld a, d
...
.printGrewLevelText
	...
	ld [wMonDataLocation], a
	call LoadMonData
	call AnimateEXPBarAgain ; ~$~ADDED: EXP bar in battle.~$~
	ld d, $1
	...
	ld a, [wCurSpecies]
	ld [wPokedexNum], a
; ~$~CHANGED: Ported KEP's port of Red++'s level-up move learning code.~$~
	ld a, [wCurEnemyLevel]
	ld c, a
	ld a, [wTempLevel]
	ld b, a
...
; ~$~REMOVED: EXP division function removed.~$~
