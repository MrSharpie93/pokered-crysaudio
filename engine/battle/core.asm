...
; ~$~FIXED~$~bug: when this is called, [hAutoBGTransferEnabled] is non-zero, so there is screen tearing
...
; ~$~CHANGED: Pressing B in wild encounters quick-selects RUN.~$~
	ld a, [wIsInBattle]
	dec a
...
; now we need to reload the enemy mon pic
	ld a, 1 ; ~$~FIXED: Sprite glitches with Minimize/Substitute.~$~
	ldh [hWhoseTurn], a
...
SwapMovesInMenu:
	ld a, [wPlayerBattleStatus3]
	bit TRANSFORMED, a
	jp nz, MoveSelectionMenu ; ~$~FIXED: No move swapping while transformed.~$~
...
.calcCriticalHitProbability
	...
	jr z, .noFocusEnergyUsed     ;~$~FIXED: Focus Energy works; normal crit rate lowered. ~$~
...
.noCarry
	...
	ld b, $ff
.SkipHighCritical
	ld a, b ; ~$~FIXED: Crits do not fail 1/256 times.~$~
...
.pickMoveLoop
	...
	jr z, .pickMoveLoop
	cp LIQUIDATION
	assert NUM_ATTACKS == LIQUIDATION ; random numbers greater than LIQUIDATION are not moves
	jr nc, .pickMoveLoop
	cp METRONOME
	jr z, .pickMoveLoop
	cp STRUGGLE ; ~$~CHANGED: Struggle doesn't have to be the last move listed.~$~
	jr z, .pickMoveLoop
	ld [hl], a
	jr ReloadMoveData
...
;~$~FIXED: Game now reports correct type effectiveness. ~$~
	ldh [hMultiplier], a
	and a  ; cp NO_EFFECT
	jr z, .gotMultiplier
...
.innerLoop ; each loop iteration writes 1 OAM entry in the column
	...
	dec b
	jr nz, .loop
;	ld de, vBackPic ; ~$~CHANGED: 48x48 back sprites.~$~
...
LoadBackSpriteUnzoomed: ; ~$~CHANGED: 48x48 back sprites.~$~
	ld a, $66
	ld de, vBackPic
	push de
