; ~$~CHANGED: Removed several VC hooks for move animations, as the animations are less severe now.~$~
...
LoadMoveAnimationTiles:
	callfar _LoadMoveAnimationTiles ; ~$~CHANGED: Now in its own file.~$~
	ret
...
FlashScreenEveryEightFrameBlocks:
	ld a, [wSubAnimCounter]
	and 14 ; ~$~CHANGED: Reduced flashing.~$~
...
AnimationFlashScreenLong:
	ld a, 2 ; ~$~CHANGED: Cycle through the palettes twice.~$~
...
AnimationFlashScreen:
	...
	ldh [rBGP], a
	ld c, 10 ; ~$~CHANGED: Reduced flashing.~$~
	call DelayFrames
...
;	dec hl
	ld a, 160 ; ~$~FIXED~$~bug, sets previous OAM entry's attribute
;	ld [hli], a
...
.PlayerNextTile
	ld a, [hl]
	add 7
; ~$~FIXED~$~This is a bug. The lower right corner tile of the mon back pic is blanked
...
.EnemyNextTile
	ld a, [hl]
	sub 7
; ~$~FIXED~$~This has the same problem as above, but it has no visible effect because
