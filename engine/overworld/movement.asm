...
.initNextMovementCounter
	...
	inc [hl]                         ;~$~FIXED~$~ note that value 0 actually makes the delay $100 (bug?)
...
;	ret ~$~FIXED: NPCs don't load into the screen corner on entering an area.~$~
...
.tilePassableLoop
	...
	add d              ; add Y delta
; ~$~FIXED: NPCs don't treat bottom row/rightmost column as ofscreen.~$~
	...
	jr nz, .upwards
	add d
	; ~$~FIXED~$~bug: these tests against $5 probably were supposed to prevent
