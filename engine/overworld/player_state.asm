...
_GetTileAndCoordsInFrontOfPlayer:
	...
	jr nz, .notFacingDown
; facing down
	ld a, 8 ; ~$~FIXED: No Cut trees returning.~$~
	ld [wTempColCoords], a
	ld a, 11
...
.notFacingUp
	...
	jr nz, .notFacingLeft
; facing left
	ld a, 6 ; ~$~FIXED: No Cut trees returning.~$~
	ld [wTempColCoords], a
	ld a, 9
	ld [wTempColCoords + 1], a
	lda_coord 6, 9
...
.notFacingLeft
	...
	jr nz, .storeTile
; facing right
	ld a, 10 ; ~$~FIXED: No Cut trees returning.~$~
	ld [wTempColCoords], a
	ld a, 9
	ld [wTempColCoords + 1], a
	lda_coord 10, 9
...
ReadTileFromVram: ; ~$~FIXED: No Cut trees returning.~$~
	;b=X window offset
	;c=Y window offset
	push bc
	ld a, [wTempColCoords]
