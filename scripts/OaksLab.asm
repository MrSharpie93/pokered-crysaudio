...
OaksLabDefaultScript: ; ~$~CHANGED: Check if you already have a starter. If you don't, determine which ball remains based on the favorite color you selected.~$~
...
OaksLabPlayerEntersLabScript: ; ~$~CHANGED: Walk to Oak, he is surprised to see another person, but offers you the remaining Pokemon anyway.~$~
	call Delay3
...
OaksLabFollowedOakScript:
	...
	ld a, OAKSLAB_OAK1
	ld [wEmotionBubbleSpriteIndex], a ; Oak's sprite
	xor a
...
;	call DelayFrame ; ~$~FIXED: Oak's lab music channel bug.~$~
...
;	dw_const OaksLabOak2Text,                     TEXT_OAKSLAB_OAK2
	...
	dw_const OaksLabEXPShareScientistText,        TEXT_OAKSLAB_SCIENTIST1
...
;	ld [wJoyIgnore], a
	...
	jp TextScriptEnd
OaksLabMonChoiceEnd:
	ld hl, OaksLabNoOtherChoiceText
	call PrintText
	jp TextScriptEnd
...
OaksLabNoOtherChoiceText:
	text_far _OaksLabNoOtherChoiceText
	text_end
...
OaksLabOak1Text: ; ~$~CHANGED: Giving Oak the parcel nets you 10 Poke Balls, since you already have the Dex.~$~
	text_asm
...
OaksLabEXPShareScientistText:
	text_asm
	ld a, [wStatusFlags1]
	bit BIT_EXP_SHARE_ACTIVE, a
	jr nz, .deactivateEXPShare
	...
	call PrintText
	ld hl, wStatusFlags1
	set BIT_EXP_SHARE_ACTIVE, [hl]
	jr .done
.nevermindThen
	ld hl, NevermindThenText
	call PrintText
	jr .done
.deactivateEXPShare
	...
	call PrintText
	ld hl, wStatusFlags1
	res BIT_EXP_SHARE_ACTIVE, [hl]
.done
	jp TextScriptEnd
	
ActivateEXPShareText:
	text_far _ActivateEXPShareText
	text_end
	
ItIsDoneText:
	text_far _ItIsDoneText
	text_end
	
NevermindThenText:
	text_far _NevermindThenText
	text_end
	
DeactivateEXPShareText:
	text_far _DeactivateEXPShareText
	text_end
